...
- Manual assignment wins while the conversation is open: if a human assigned the conversation to a specific agent, the router locks it and never reassigns it while it stays open, even when that agent is offline. The router only manages an agent it assigned itself (it may still move that agent to an online colleague while the conversation is auto-handled). A "manual" assignment is detected whenever the current assignee is not the agent the router last assigned.
